...
- Minimal `stash list` support over `refs/stash` reflogs, including default
  ...
  output-indicator, submodule, color-moved including boolean aliases, pickaxe, ita, and rewrite no-op
...
- Minimal `diff --name-status`, `diff --name-only`, HEAD comparison,
  ...
  name-output no-op `--ext-diff` / `--textconv`,
  ...
  rewrite-detection controls `-B` / `--break-rewrites` for name output,
  `--relative` / `--no-relative`, `-R`, literal `-S` pickaxe name output
  including `--pickaxe-all`, and `--find-object` name output,
  ...
  `--find-copies` / `--find-copies-harder` / `--no-find-copies-harder`,
  similarity-threshold validation, no-op `-l<n>` rename-limit parsing
