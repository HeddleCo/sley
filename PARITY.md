...
- Minimal `stash list` support over `refs/stash` reflogs, including default
  ...
  diff-filter, explicit-stash, and untracked-display output, plus
...
- Upstream Git test script import is not implemented; the current harness covers
  ...
  `stash show` default raw/stat/compact-summary/numstat/shortstat/summary/name/name-status/patch/quiet/exit-code/diff-filter/untracked output, `stash store` ref/reflog writes,
