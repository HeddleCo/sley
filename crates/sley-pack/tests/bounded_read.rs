...
use sley_pack::{
    BoundedPackDecoder, PackFile, PackLimitKind, PackReadError, PackReadLimits, PackWriteOptions,
    RefDeltaBase, SlicePackSource,
};
...
fn bounded_decoder_matches_existing_decoder_for_ofs_and_in_pack_ref_deltas() {
    ...
    for prefer_ofs_delta in [true, false] {
        ...
        let parsed = PackFile::parse_sha1(&written.pack).expect("parse ground truth");
        ...
        for expected in parsed.entries.iter().rev() {
            let decoded = decoder
                .read_object_at(expected.entry.offset, |oid| {
                    Ok(offsets.get(oid).copied().map(RefDeltaBase::InPack))
                })
                .expect("decode object");
            assert_eq!(*decoded.object, expected.object);
            saw_delta |= decoded.stats.delta_depth > 0;
            assert!(decoded.stats.peak_materialized_bytes <= limits.max_materialized_bytes);
            assert!(decoded.stats.cached_bytes <= limits.max_cached_bytes);
        }
        assert!(saw_delta, "targeted reads must exercise delta resolution");
    }
}
...
fn bounded_decoder_preserves_external_ref_delta_resolution() {
    ...
    let decoded = decoder
        .read_object_at(written.entries[0].offset, |oid| {
            Ok((*oid == base_oid).then(|| RefDeltaBase::External(Arc::new(base.clone()))))
        })
...
fn deep_ofs_chain_on_small_stack_returns_typed_depth_limit() {
    ...
    let error = std::thread::Builder::new()
        ...
        .spawn(move || {
            ...
            .expect("open decoder");
            decoder
                .read_object_at(target_offset, |_| Ok(None))
...
fn cache_is_byte_bounded_evicts_and_can_be_cleared() {
    ...
    for entry in &written.entries {
        let decoded = decoder
            .read_object_at(entry.offset, |_| Ok(None))
            .expect("decode object");
...
fn malformed_and_truncated_sources_return_pack_errors() {
    ...
    assert!(matches!(
        decoder.read_object_at(entry_offset as u64, |_| Ok(None)),
