...
const GIT_RS: &str = env!("CARGO_BIN_EXE_git-rs");
...
fn autocrlf_true_round_trip_matches_git() {
    ...
    assert_eq!(
        run_env("git", &root.join("ref"), &["rev-parse", "HEAD:f.txt"]).stdout,
        run_env(GIT_RS, &root.join("cand"), &["rev-parse", "HEAD:f.txt"]).stdout,
        "blob OID differs between git and git-rs under autocrlf"
    );
...
fn no_filter_default_is_passthrough() {
    ...
    assert_eq!(
        run_env("git", &reference, &["rev-parse", "HEAD:m.bin"]).stdout,
