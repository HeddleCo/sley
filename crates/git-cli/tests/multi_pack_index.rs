...
fn create_named_pack(root: &Path, body: &[u8]) -> (String, String) {
    let oid = run_success_with_stdin("git", root, &["hash-object", "-w", "--stdin"], body);
...
fn create_named_pack_in_object_dir(root: &Path, object_dir: &str, body: &[u8]) -> (String, String) {
    ...
    let oid =
        run_success_with_env_and_stdin("git", root, &["hash-object", "-w", "--stdin"], &envs, body);
    let oid = String::from_utf8(oid)
        .expect("object id is utf8")
        .trim()
        .to_string();
...
fn multi_pack_index_write_matches_upstream_and_verifies() {
    ...
    let result = (|| {
        ...
        assert!(midx_path.exists(), "git-rs did not write multi-pack-index");
        run_success("git", &root, &["multi-pack-index", "verify"]);
        assert_eq!(
            run_success("git", &root, &["cat-file", "-p", &first]),
            b"first midx object\n"
        );
        assert_eq!(
...
fn multi_pack_index_git_object_directory_default_matches_upstream_git() {
    ...
    let result = (|| {
        ...
        let actual_verify =
            run_with_env(env!("CARGO_BIN_EXE_git-rs"), &actual, &verify_args, &envs);
        ...
        let actual_expire =
            run_with_env(env!("CARGO_BIN_EXE_git-rs"), &actual, &expire_args, &envs);
...
fn multi_pack_index_write_stdin_packs_matches_upstream() {
    ...
    let result = (|| {
        ...
        let expected = run_with_stdin("git", &root, &args, stdin.as_bytes());
        let midx_path = root
            .join(".git")
            .join("objects")
            .join("pack")
        ...
        let actual = run_with_stdin(env!("CARGO_BIN_EXE_git-rs"), &root, &args, stdin.as_bytes());
        assert_same_output(actual, expected, &args);
        run_success("git", &root, &["multi-pack-index", "verify"]);
        assert!(midx_path.exists(), "git-rs did not write multi-pack-index");
        assert!(
...
fn multi_pack_index_verify_matches_upstream_git() {
    ...
    let result = (|| {
        ...
        run_success("git", &root, &["multi-pack-index", "write"]);
