...
//! subcommands through `git` in one and `git-rs` in the other, asserting that
...
/// so that commit object ids are reproducible across `git` and `git-rs`.
...
/// A pair of identical repositories: one driven by `git`, one by `git-rs`.
struct RepoPair {
    git_repo: PathBuf,
    rs_repo: PathBuf,
}
