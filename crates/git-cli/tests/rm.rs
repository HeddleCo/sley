...
fn rm_tracked_paths_match_upstream_git() {
    ...
    let result = (|| {
        ...
        assert!(
            ...
            "git-rs rm left tracked file"
        );
        assert!(
            !rust.join("dir/nested.txt").exists(),
            "git-rs rm left nested tracked file"
        );
        assert_eq!(
...
fn rm_cached_keeps_worktree_and_removes_index_like_upstream_git() {
    ...
    let result = (|| {
        ...
        assert_eq!(
            ...
            b"worktree\n",
            "git-rs rm --cached changed worktree file"
        );
        assert_eq!(
            git(&rust, &["diff", "--cached", "--name-status"]),
            git(&upstream, &["diff", "--cached", "--name-status"]),
...
fn rm_dry_run_reports_paths_without_removing_like_upstream_git() {
    ...
    let result = (|| {
        ...
        assert!(rust.join("file.txt").exists(), "git-rs rm -n removed file");
        assert!(
            rust.join("dir/nested.txt").exists(),
            "git-rs rm -n removed nested file"
        );
        assert_eq!(
            git(&rust, &["diff", "--cached", "--name-status"]),
            git(&upstream, &["diff", "--cached", "--name-status"]),
...
fn rm_no_dry_run_overrides_dry_run_like_upstream_git() {
    ...
    let result = (|| {
        ...
        assert!(!rust.join("file.txt").exists(), "git-rs rm left file");
        assert_eq!(
            git(&rust, &["status", "--short"]),
            git(&upstream, &["status", "--short"]),
...
fn rm_no_cached_overrides_cached_like_upstream_git() {
    ...
    let result = (|| {
        ...
        assert!(
            !rust.join("file.txt").exists(),
            "git-rs rm --no-cached left worktree file"
        );
        assert_eq!(
            git(&rust, &["status", "--short"]),
            git(&upstream, &["status", "--short"]),
...
fn rm_no_force_overrides_force_like_upstream_git() {
    ...
    let result = (|| {
        ...
        assert!(
            rust.join("file.txt").exists(),
            "git-rs rm --no-force removed modified file"
        );
    })();
    let _ = fs::remove_dir_all(&root);
    result
...
fn rm_combined_short_options_match_upstream_git() {
    ...
    let result = (|| {
        ...
        assert!(
            rust.join("dir/nested.txt").exists(),
            "git-rs rm -rn removed nested file"
        );
        assert_eq!(
            git(&rust, &["diff", "--cached", "--name-status"]),
            git(&upstream, &["diff", "--cached", "--name-status"]),
...
fn rm_ignore_unmatch_skips_missing_paths_like_upstream_git() {
    ...
    let result = (|| {
        ...
        assert!(
            rust.join("file.txt").exists(),
            "git-rs rm --ignore-unmatch removed tracked file"
        );
        assert_eq!(
            git(&rust, &["diff", "--cached", "--name-status"]),
            git(&upstream, &["diff", "--cached", "--name-status"]),
...
fn rm_ignore_unmatch_removes_matched_paths_like_upstream_git() {
    ...
    let result = (|| {
        ...
        assert!(
            !rust.join("file.txt").exists(),
            "git-rs rm --ignore-unmatch left matched tracked file"
        );
        assert_eq!(
            git(&rust, &["diff", "--cached", "--name-status"]),
            git(&upstream, &["diff", "--cached", "--name-status"]),
...
fn rm_pathspec_from_file_matches_upstream_git() {
    ...
    let result = (|| {
        ...
        assert!(
            !rust.join("file.txt").exists(),
            "git-rs rm --pathspec-from-file left tracked file"
        );
        assert!(
            !rust.join("dir/nested.txt").exists(),
            "git-rs rm --pathspec-from-file left nested file"
        );
        assert_eq!(
            git(&rust, &["diff", "--cached", "--name-status"]),
            git(&upstream, &["diff", "--cached", "--name-status"]),
...
fn rm_pathspec_file_nul_matches_upstream_git() {
    ...
    let result = (|| {
        ...
        assert!(
            !rust.join("file.txt").exists(),
            "git-rs rm --pathspec-file-nul left tracked file"
        );
        assert!(
            !rust.join("dir/nested.txt").exists(),
            "git-rs rm --pathspec-file-nul left nested file"
        );
        assert_eq!(
            git(&rust, &["diff", "--cached", "--name-status"]),
            git(&upstream, &["diff", "--cached", "--name-status"]),
...
fn rm_force_removes_modified_paths_like_upstream_git() {
    ...
    let result = (|| {
        ...
        assert!(!rust.join("file.txt").exists(), "git-rs rm -f left file");
        assert_eq!(
            git(&rust, &["diff", "--cached", "--name-status"]),
            git(&upstream, &["diff", "--cached", "--name-status"]),
