...
fn log_minimal_format_matches_upstream_git() {
    ...
    let result = (|| {
        ...
        for rev in ["HEAD", "refs/tags/v1.0"] {
            ...
            assert_eq!(actual, expected, "git-rs log output differed for {rev}");
        }
        for args in [
            vec!["-1", "HEAD"],
            ...
            assert_eq!(actual, expected, "git-rs log output differed for {args:?}");
        }
        for (args, stdin) in [
            ...
            assert_eq!(
                ...
                "git-rs log output differed for {args:?} with stdin {:?}",
