    ...
    let result = (|| {
        ...
        for args in [
            ...
            assert_eq!(
                ...
                "git-rs output differed for {args:?}"
            );
        }
    })();
    let _ = fs::remove_dir_all(&root);
...
fn ls_files_ignored_and_exclude_standard_match_upstream_git() {
    ...
    let result = (|| {
        ...
        let actual = run_status(env!("CARGO_BIN_EXE_git-rs"), &rust, &args);
        assert_eq!(
            actual, expected,
            "git-rs status/output differed for {args:?}"
        );
    })();
    let _ = fs::remove_dir_all(&root);
    result
...
fn ls_files_default_global_excludes_match_upstream_git() {
    ...
    let result = (|| {
        ...
        for args in [
            ...
            assert_eq!(
                actual_output, expected_output,
                "git-rs XDG global excludes output differed for {args:?}"
            );
        }
        ...
        assert_eq!(
            actual_output, expected_output,
            "git-rs core.excludesFile override differed"
        );
        ...
        assert_eq!(
            actual_output, expected_output,
            "git-rs HOME fallback global excludes output differed"
        );
    })();
    let _ = fs::remove_dir_all(&root);
    result
...
fn ls_files_error_unmatch_matches_upstream_git() {
    ...
    let result = (|| {
        ...
        for args in [
            ...
            assert_eq!(
                actual, expected,
                "git-rs status/stdout/stderr differed for {args:?}"
            );
        }
    })();
    let _ = fs::remove_dir_all(&root);
