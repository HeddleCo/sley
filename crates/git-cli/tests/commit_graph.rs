...
fn commit_graph_write_reachable_writes_upstream_verifiable_graph() {
    ...
    let result = (|| {
        ...
        assert!(
            ...
            "git-rs {args:?} failed with status {:?}\nstdout:\n{}\nstderr:\n{}",
            actual.status.code(),
        ...
        assert!(graph_path.exists(), "git-rs did not write commit-graph");
...
fn commit_graph_write_without_selector_matches_upstream_noop() {
    ...
    let result = (|| {
        ...
        let actual_output = run(env!("CARGO_BIN_EXE_git-rs"), &empty_actual, &args);
        assert_same_output(actual_output, expected_output, &args);
    })();
    let _ = fs::remove_dir_all(&root);
    result
