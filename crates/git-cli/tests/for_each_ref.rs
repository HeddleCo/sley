...
fn git_rs_raw(cwd: &Path, args: &[&str]) -> Output {
    run_raw(env!("CARGO_BIN_EXE_git-rs"), cwd, args)
}
...
fn git_rs_raw_with_stdin(cwd: &Path, args: &[&str], stdin: &[u8]) -> Output {
    run_raw_with_stdin(env!("CARGO_BIN_EXE_git-rs"), cwd, args, stdin)
}
...
fn for_each_ref_minimal_formats_match_upstream_git() {
    ...
    let result = (|| {
        ...
        assert_eq!(
            actual, expected,
            "git-rs output differed for core.abbrev-driven objectname:short"
        );
        ...
        for args in [
            ...
            assert!(
                ...
                "git-rs unexpectedly accepted {args:?}"
            );
        }
    })();
