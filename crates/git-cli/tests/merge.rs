...
//! ids are deterministic), copy the repo, then run `git-rs merge` in one copy and
...
//! stdout includes a diffstat that git-rs does not emit yet, so stdout is checked
...
/// object ids are reproducible across `git` and `git-rs`.
...
fn merge_conflict_matches_git() {
    ...
    assert_eq!(
        rs_out.status.code(),
        Some(1),
        "git-rs merge conflict exit differs: {}",
        String::from_utf8_lossy(&rs_out.stderr)
    );
    ...
    assert!(
        git(&candidate, &["rev-parse", "--verify", "MERGE_HEAD"])
            .status
            .success(),
        "git-rs did not record MERGE_HEAD"
    );
