...
fn prepare_repo_with_linked_worktree(repo: &Path, linked: &Path) {
    ...
    run_success("git", repo, &["branch", "topic"]);
...
fn worktree_list_matches_upstream_git() {
    ...
    let result = (|| {
        ...
        for args in [
            ...
            let expected = run("git", &repo, &args);
            let actual = run(env!("CARGO_BIN_EXE_git-rs"), &repo, &args);
...
fn worktree_list_detached_head_matches_upstream_git() {
    ...
    let result = (|| {
        ...
        run_success("git", &repo, &["init", "-q"]);
        run_success_with_identity(&repo, &["commit", "--allow-empty", "-qm", "initial"]);
        run_success("git", &repo, &["checkout", "-q", "--detach", "HEAD"]);
...
fn worktree_add_branch_detach_default_and_lock_match_upstream_git() {
    ...
    let result = (|| {
        ...
        let expected_list = run("git", &upstream, &["worktree", "list", "--porcelain"]);
        let actual_list = run(
            env!("CARGO_BIN_EXE_git-rs"),
            &actual,
...
fn worktree_prune_skips_locked_stale_worktrees_like_upstream_git() {
    ...
    let result = (|| {
        ...
        let expected = run("git", &upstream, &lock_args);
        let actual_output = run(env!("CARGO_BIN_EXE_git-rs"), &actual, &lock_args);
        assert_same_output_normalized(
            actual_output,
            expected,
            &lock_args,
...
fn worktree_remove_clean_and_error_paths_match_upstream_git() {
    ...
    let result = (|| {
        ...
        for args in [
            ...
            let expected = run("git", &upstream_errors, &args);
            let actual_output = run(env!("CARGO_BIN_EXE_git-rs"), &actual_errors, &args);
            assert_same_output_normalized(
                actual_output,
                expected,
                &args,
...
fn worktree_remove_force_dirty_and_locked_match_upstream_git() {
    ...
    let result = (|| {
        ...
        run_success("git", &upstream, &["branch", "locked"]);
        run_success("git", &actual, &["branch", "locked"]);
        run_success(
            "git",
            &upstream,
            &[
...
fn worktree_move_clean_dirty_and_directory_destination_match_upstream_git() {
    ...
    let result = (|| {
        ...
        let expected = run("git", &upstream, &list_args);
        let actual_output = run(env!("CARGO_BIN_EXE_git-rs"), &actual, &list_args);
        assert_same_output_normalized(
            actual_output,
            expected,
            &list_args,
        ...
        let args = vec!["worktree", "move", "../linked", "../moved"];
        let expected = run("git", &upstream_dirty, &args);
        let actual_output = run(env!("CARGO_BIN_EXE_git-rs"), &actual_dirty, &args);
        assert_same_output_normalized(
            actual_output,
            expected,
            &args,
        ...
        let expected = run("git", &upstream_dir, &args);
        let actual_output = run(env!("CARGO_BIN_EXE_git-rs"), &actual_dir, &args);
        assert_same_output_normalized(
            actual_output,
            expected,
            &args,
...
fn worktree_repair_updates_moved_worktree_metadata_like_upstream_git() {
    ...
    let result = (|| {
        ...
        let expected = run("git", &upstream_noarg_moved, &no_args);
        let actual_output = run(env!("CARGO_BIN_EXE_git-rs"), &actual_noarg_moved, &no_args);
        assert_same_output_normalized(
            actual_output,
            expected,
            &no_args,
