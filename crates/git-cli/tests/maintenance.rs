...
fn apply_modifies_worktree_like_git() {
    ...
    assert_eq!(
        ...
        "git-rs apply produced different bytes than git apply"
    );
...
fn apply_check_succeeds_for_clean_patch() {
    ...
    assert!(
        out.status.success(),
        "git-rs apply --check rejected a clean patch: {}",
...
fn repack_d_keeps_repository_complete() {
    ...
    assert!(
        out.status.success(),
        "git-rs repack -d failed: {}",
        String::from_utf8_lossy(&out.stderr)
    );
    let fsck = git(&repo, &["fsck", "--no-progress"]);
    assert!(
        fsck.status.success(),
        "git fsck failed after git-rs repack -d: {}",
        String::from_utf8_lossy(&fsck.stderr)
    );
