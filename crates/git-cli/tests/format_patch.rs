...
//! lines are deterministic), then asserts that `git-rs format-patch ...` produces
...
//! same version git-rs targets: the mbox `-- \n<version>` signature trailer
...
/// the one git-rs targets, because the patch signature trailer embeds that
/// version string.
fn interop_enabled() -> bool {
    ...
    // `git --version` prints "git version X.Y.Z"; git-rs mirrors that.
    let Ok(rs_version) = Command::new(env!("CARGO_BIN_EXE_git-rs"))
        .arg("version")
        .output()
    else {
        return false;
...
/// Assert `git-rs format-patch <args>` matches `git format-patch <args>` on
...
fn assert_same_files(repo: &Path, extra: &[&str]) {
    ...
    for name in &g_names {
        ...
        let r_bytes = fs::read(rs_dir.join(name)).expect("read git-rs patch");
        assert_eq!(
