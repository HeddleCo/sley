...
fn cat_file_reads_alternate_object_directories_like_upstream_git() {
    ...
    let result = (|| {
        ...
        assert_same_output(
            run_output_with_stdin(env!("CARGO_BIN_EXE_git-rs"), &actual, &args, &[]),
        ...
        assert_same_output(
            run_output_with_env(env!("CARGO_BIN_EXE_git-rs"), &actual, &args, &actual_env),
...
fn cat_file_batch_all_objects_git_object_directory_matches_upstream_git() {
    ...
    let result = (|| {
        ...
        for args in [
            ...
            assert_same_output(
                run_output_with_env(env!("CARGO_BIN_EXE_git-rs"), &actual, &args, &envs),
...
fn git_rs(cwd: &Path, args: &[&str], stdin: &[u8]) -> Vec<u8> {
    run_with_stdin(env!("CARGO_BIN_EXE_git-rs"), cwd, args, stdin)
}
...
fn cat_file_batch_modes_match_upstream_git() {
    ...
    let result = (|| {
        ...
        for args in [
            ...
            let actual = run_output_with_stdin(env!("CARGO_BIN_EXE_git-rs"), &root, &args, b"");
        ...
        for mode in ["--batch-check", "--batch"] {
            ...
            assert_eq!(actual, expected, "git-rs {mode} output differed");
        }
        for args in [
        ...
        for (args, input) in [
            ...
            assert_eq!(
                actual, expected,
                "git-rs packed output differed for {args:?}"
            );
        }
        for args in [
            vec!["cat-file", "--batch-check", "--batch-all-objects"],
...
fn cat_file_batch_storage_atoms_match_upstream_for_delta_pack() {
    ...
    let result = (|| {
        ...
        assert_eq!(
            actual, expected,
            "git-rs delta storage atom output differed"
        );
    })();
    let _ = fs::remove_dir_all(&root);
    result
