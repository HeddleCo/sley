...
fn clone_local_repository_matches_upstream_git() {
    ...
    let result = (|| {
        ...
        let expected = run("git", &root, &["clone", "-q", source_arg, expected_arg]);
        let actual = run(
        ...
        for key in [
            ...
            let expected = run("git", &expected_repo, &["config", "--get", &key]);
            let actual = run("git", &actual_repo, &["config", "--get", &key]);
...
fn clone_local_repository_default_directory_matches_upstream_git() {
    ...
    let result = (|| {
        ...
        let expected = run("git", &expected_root, &["clone", "-q", &source_arg]);
        let actual = run(
            env!("CARGO_BIN_EXE_git-rs"),
            &actual_root,
            &["clone", "-q", &source_arg],
...
fn clone_local_repository_bundle_uri_flags_match_upstream_git() {
    ...
    let result = (|| {
        ...
        run_success("git", &source, &["bundle", "create", bundle_arg, "--all"]);
...
fn clone_local_repository_separate_git_dir_flags_match_upstream_git() {
    ...
    let result = (|| {
        ...
        for case in cases {
            ...
            let expected = run("git", &root, &expected_arg_refs);
            let actual = run(env!("CARGO_BIN_EXE_git-rs"), &root, &actual_arg_refs);
...
fn clone_local_repository_reference_flags_match_upstream_git() {
    ...
    let result = (|| {
        ...
        run_success("git", &reference, &["add", "reference.txt"]);
        run_success(
            "git",
            &reference,
            &[
