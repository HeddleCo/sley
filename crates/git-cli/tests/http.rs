//! Integration tests for git-rs's smart-HTTP transport.
//!
...
//! `git-rs clone`/`fetch`/`ls-remote`/`push` over `http://` and compare the
...
fn ls_remote_http_matches_upstream() {
    ...
    let result = (|| {
        ...
        assert!(
            ...
            "git-rs ls-remote over http failed\nstdout:\n{}\nstderr:\n{}",
        ...
        assert_eq!(
            ...
            "git-rs ls-remote refs differed from upstream over http"
        );
    })();
...
fn fetch_http_incremental() {
    ...
    let result = (|| {
        ...
        assert_eq!(
            after_head, new_head,
            "git-rs fetch did not update origin/main to the new upstream commit"
        );
