...
//! `git-rs` build and asserts identical stdout, stderr, and exit code. A handful
...
fn assert_same_stdin(cwd: &Path, args: &[&str], stdin: &[u8]) {
    let expected = run_env_stdin("git", cwd, args, stdin);
    let actual = run_env_stdin(env!("CARGO_BIN_EXE_git-rs"), cwd, args, stdin);
    assert_eq!(
        String::from_utf8_lossy(&actual.stdout),
        String::from_utf8_lossy(&expected.stdout),
