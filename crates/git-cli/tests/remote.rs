...
fn remote_add_list_get_url_and_remove_match_upstream_git() {
    ...
    let result = (|| {
        ...
        assert_eq!(actual, expected, "git-rs output differed after remove");
        ...
        assert_eq!(actual, expected, "git-rs output differed after set-url");
        ...
        assert_eq!(
            actual, expected,
            "git-rs output differed after old-url replacement"
        );
        ...
        let expected = git(&upstream, &["remote", "get-url", "--all", "origin"]);
        let actual = git_rs(&rust, &["remote", "get-url", "--all", "origin"]);
        assert_eq!(actual, expected, "git-rs output differed after --delete");
        ...
        assert_eq!(
            actual, expected,
            "git-rs fetch refspec config differed after set-branches"
        );
        for args in [
            ...
            assert_eq!(
                actual, expected,
                "git-rs remote show output differed for {args:?}"
            );
        }
        ...
        assert_eq!(
            actual, expected,
            "git-rs verbose output differed after pushurl"
        );
        ...
        assert_eq!(
            actual, expected,
            "git-rs output differed after pushurl delete"
        );
        ...
        for args in [
            ...
            assert_eq!(
                actual, expected,
                "git-rs rewritten remote URL output differed for {args:?}"
            );
        }
        ...
        for args in [
            ...
            assert_eq!(
                actual, expected,
                "git-rs output differed after rename for {args:?}"
            );
        }
        ...
        assert_eq!(
            actual, expected,
            "git-rs fetch refspec config differed after rename"
        );
        let expected = git(
            &upstream,
            &[
        ...
        assert_eq!(
            actual, expected,
            "git-rs remote-tracking refs differed after rename"
        );
        let expected = git(
            &upstream,
        ...
        assert_eq!(
            actual, expected,
            "git-rs branch/remote config differed after rename"
        );
        assert!(
            !rust.join(".git/refs/remotes/origin").exists(),
            "git-rs left old remote-tracking ref directory after rename"
        );
        ...
        assert_eq!(
            actual, expected,
            "git-rs remote show output differed with local remote refs"
        );
        ...
        assert_eq!(actual, expected, "git-rs remote HEAD target differed");
        ...
        assert!(
            ...
            "git-rs did not delete remote HEAD symbolic ref"
        );
    })();
...
fn remote_rename_moves_packed_remote_tracking_refs_match_upstream_git() {
    ...
    let result = (|| {
        ...
        assert_eq!(
            actual, expected,
            "git-rs packed remote-tracking refs differed after rename"
        );
        let packed_refs =
            fs::read_to_string(rust.join(".git/packed-refs")).expect("read packed-refs");
        assert!(
            !packed_refs.contains("refs/remotes/origin/"),
            "git-rs left old packed remote-tracking refs after rename"
        );
    })();
    let _ = fs::remove_dir_all(&root);
    result
...
fn remote_remove_cleans_refs_and_branch_config_match_upstream_git() {
    ...
    let result = (|| {
        ...
        assert_same_output(
            run_output(env!("CARGO_BIN_EXE_git-rs"), &rust, &args),
        ...
        assert_eq!(
            actual, expected,
            "git-rs remote-tracking refs differed after remove"
        );
        let expected = git(
            &upstream,
        ...
        assert_eq!(
            actual, expected,
            "git-rs branch config differed after remote remove"
        );
        ...
        assert_eq!(
            actual, expected,
            "git-rs remote output differed after remove"
        );
        let packed_refs =
            fs::read_to_string(rust.join(".git/packed-refs")).expect("read packed-refs");
        assert!(
            !packed_refs.contains("refs/remotes/origin/"),
            "git-rs left old packed remote-tracking refs after remove"
        );
    })();
    let _ = fs::remove_dir_all(&root);
    result
...
fn remote_set_branches_negations_match_upstream_git() {
    ...
    let result = (|| {
        ...
        for args in [
            ...
            assert_eq!(
                actual, expected,
                "git-rs fetch refspecs differed after {args:?}"
            );
        }
    })();
    let _ = fs::remove_dir_all(&root);
...
fn remote_set_head_auto_local_remote_matches_upstream_git() {
    ...
    let result = (|| {
        ...
        for args in [
            ...
            assert_eq!(
                actual, expected,
                "git-rs remote HEAD target differed after {args:?}"
            );
        }
        ...
        assert_eq!(
            actual, expected,
            "git-rs remote HEAD target differed after changed auto set-head"
        );
    })();
    let _ = fs::remove_dir_all(&root);
    result
...
fn remote_prune_local_remote_matches_upstream_git() {
    ...
    let result = (|| {
        ...
        assert_same_output(
            run_output(env!("CARGO_BIN_EXE_git-rs"), &rust, &dry_run_args),
        ...
        assert_eq!(
            actual, expected,
            "git-rs dry-run prune changed remote-tracking refs"
        );

        let dry_run_reset_args = ["remote", "prune", "--dry-run", "--no-dry-run", "origin"];
        assert_same_output(
            run_output(env!("CARGO_BIN_EXE_git-rs"), &rust, &dry_run_reset_args),
        ...
        assert_same_output(
            run_output(env!("CARGO_BIN_EXE_git-rs"), &rust, &no_dry_run_reset_args),
            run_output("git", &upstream, &no_dry_run_reset_args),
            &no_dry_run_reset_args,
        );
        let prune_args = ["remote", "prune", "origin"];
        assert_same_output(
            run_output(env!("CARGO_BIN_EXE_git-rs"), &rust, &prune_args),
        ...
        assert_eq!(
            actual, expected,
            "git-rs remote-tracking refs differed after prune"
        );
    })();
    let _ = fs::remove_dir_all(&root);
    result
...
fn remote_add_track_branches_match_upstream_git() {
    ...
    let result = (|| {
        ...
        assert_eq!(
            actual, expected,
            "git-rs tracked remote fetch refspecs differed"
        );
        let expected = git(&upstream, &["remote", "-v"]);
        let actual = git_rs(&rust, &["remote", "-v"]);
        assert_eq!(actual, expected, "git-rs verbose remote output differed");
    })();
    let _ = fs::remove_dir_all(&root);
    result
}
