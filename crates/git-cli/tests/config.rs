...
fn assert_status_stdout_stderr_match(upstream: &Path, rust: &Path, args: &[&str]) {
    ...
    assert_eq!(
        ...
        "git-rs status differed for {args:?}\nstdout:\n{}\nstderr:\n{}",
        String::from_utf8_lossy(&actual.stdout),
        String::from_utf8_lossy(&actual.stderr)
    );
    assert_eq!(
        actual.stdout, expected.stdout,
        "git-rs stdout differed for {args:?}"
    );
    assert_eq!(
        actual.stderr, expected.stderr,
        "git-rs stderr differed for {args:?}"
    );
}
...
fn config_get_set_add_and_unset_match_upstream_git() {
    ...
    let result = (|| {
        ...
        assert_eq!(
            ...
            "git-rs unset-all output differed"
        );
        assert_status_stdout_stderr_match(
            &upstream,
            &rust,
