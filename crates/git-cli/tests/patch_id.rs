...
//! `git-rs patch-id` and asserts stdout, stderr, and the exit code match
...
/// (both `git` and `git-rs`) so commit object ids are reproducible.
...
/// Assert git and git-rs produce byte-identical stdout/stderr and the same exit
