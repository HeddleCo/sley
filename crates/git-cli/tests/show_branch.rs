...
//! that `git-rs show-branch ...` produces byte-identical stdout, stderr, and
...
fn git_rs(cwd: &Path, args: &[&str]) -> Output {
    run_env_dated(env!("CARGO_BIN_EXE_git-rs"), cwd, args, &base_date())
}
...
/// Assert `git-rs show-branch <args>` matches `git show-branch <args>` on
