//! Differential tests for `git read-tree`, comparing the `git-rs` binary
//! against the system `git`.
//!
//! Each test stands up two identical repositories (one driven by upstream
//! `git`, one by `git-rs`), runs the same `read-tree` invocation in both, and
...
/// Path to the `git-rs` binary under test.
...
/// `git-rs` repository, returning `(upstream, rust, tree1, tree2)`.
///
...
fn read_tree_reset_update_resets_index_and_worktree() {
    ...
    let result = std::panic::catch_unwind(|| {
        ...
        assert!(!rust.join("c.txt").exists(), "git-rs left c.txt on disk");
