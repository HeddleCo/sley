...
//! git-rs output (status + stdout + stderr) against the system `git` binary
...
/// match between the git-rs and system-git repositories.
...
/// sequence of mutating commands can be replayed against git and git-rs and the
/// objects compared.
...
/// The system git runs in the "-expected" repo and git-rs in the "-actual"
...
/// both repos (so the on-disk object graph git-rs produced matches git's).
