    ...
    let result = (|| {
        ...
        for args in [
            ...
            assert_eq!(
                git(&upstream, args),
                git_rs(&rust, args),
                "git-rs clean output differed for {args:?}"
            );
        }
...
fn clean_requires_force_or_dry_run_like_upstream_git() {
    ...
    let result = (|| {
        ...
        let actual = run_output(env!("CARGO_BIN_EXE_git-rs"), &root, &["clean"]);
        assert_eq!(
            actual.status.code(),
            expected.status.code(),
            "git-rs clean status differed"
        );
    })();
    let _ = fs::remove_dir_all(&root);
    result
...
fn clean_require_force_false_matches_upstream_git() {
    ...
    let result = (|| {
        ...
        assert_eq!(
            git(&upstream, &["clean"]),
            git_rs(&rust, &["clean"]),
            "git-rs clean output differed with clean.requireForce=false"
        );
        assert_eq!(
            files(&rust),
            files(&upstream),
