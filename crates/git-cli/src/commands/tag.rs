...
fn tag_contains(
    ...
    let Ok(target) = git_rev::peel_to_commit(db, format, contains) else {
        return Ok(!include);
    };
    let contains = git_rev::peel_to_commit(db, format, oid)
        .ok()
        .map(|tip| {
            git_rev::walk_commits(db, format, [tip])
...
fn tag_merged(
    ...
    let merged = git_rev::peel_to_commit(db, format, merged)?;
    let reachable = git_rev::walk_commits(db, format, [merged])?
        .into_iter()
    ...
    let merged = git_rev::peel_to_commit(db, format, oid)
...
fn tag_points_at(
    ...
    Ok(git_rev::peel_tags(db, format, oid)? == *points_at)
}
