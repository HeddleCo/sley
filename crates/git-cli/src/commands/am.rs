...
fn apply_one_patch(
    ...
    let file_patches = git_diff_merge::parse_unified_patch(&patch.diff)?;
...
fn try_straight_apply(
    worktree_root: &Path,
    file_patches: &[git_diff_merge::FilePatch],
    ...
    for patch in file_patches {
        ...
        let content = match git_diff_merge::apply_file_patch(&base, patch) {
            ...
            git_diff_merge::ApplyOutcome::Rejected => return Ok(None),
        };
        if patch.is_delete {
...
fn create_am_commit(
    ...
    let mut db = FileObjectDatabase::from_git_dir(common_git_dir, format);
    let new_oid = git_sequencer::create_commit(
        &mut db,
    ...
    tx.commit()?;
    git_worktree::reset_index_and_worktree_to_commit(worktree_root, git_dir, format, &new_oid)?;
    Ok(())
}
...
fn am_author_identity(patch: &AmPatch) -> Result<Vec<u8>> {
    ...
    git_sequencer::format_commit_identity(&patch.author_name, &patch.author_email, &date)
}
...
fn apply_three_way(
    ...
    for file in file_patches {
        ...
        let inherited_mode = ours_map
            ...
            .map(|(mode, _)| *mode)
            .unwrap_or(0o100644);
        match git_diff_merge::apply_file_patch(&base_bytes, file) {
            git_diff_merge::ApplyOutcome::Applied(post) => {
...
fn am_abort(
        ...
        && let Ok(oid) = ObjectId::from_hex(format, safety)
    {
        ...
        tx.commit()?;
        git_worktree::reset_index_and_worktree_to_commit(worktree_root, git_dir, format, &oid)?;
    }
    finish_am(state_dir)
}
...
fn am_skip(
    ...
    git_worktree::reset_index_and_worktree_to_commit(worktree_root, git_dir, format, &head_oid)?;
