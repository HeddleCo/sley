...
fn show_commit(
    ...
    let record = git_rev::CommitRecord {
        oid: oid.clone(),
...
fn parse_show_args(args: &[String]) -> Result<ShowOptions> {
    ...
    while let Some(arg) = iter.next() {
        ...
        match arg.as_str() {
            ...
            // These influence rendering details git-rs does not yet model; accept
...
fn parse_pretty_value(value: &str) -> Result<ShowCommitFormat> {
    ...
    match value {
        ...
        // Built-in named layouts git-rs does not yet render. Reject explicitly
