...
//!     `git_diff_merge::diff_name_status_trees_*`.
...
fn resolve_arg_request(
    ...
    if revs.len() == 1 {
        ...
        let left_tree = git_rev::peel_to_tree(db, format, &left)?;
        let right_tree = git_rev::peel_to_tree(db, format, &right)?;
        Ok(DiffRequest {
            left: Some(left_tree),
...
fn single_commit_request(
    ...
    let left = match commit.parents.first() {
        Some(parent) => Some(git_rev::peel_to_tree(db, format, parent)?),
        None => None,
    };
...
fn parse_stdin_request(
    ...
    if let Some(second) = second {
        ...
        let (Ok(left_tree), Ok(right_tree)) = (
            git_rev::peel_to_tree(db, format, &left),
            git_rev::peel_to_tree(db, format, &right),
        ) else {
            return Ok(skip_echo(line));
        };
...
///
///   * Recursive: delegate to `git_diff_merge`, which flattens subtrees into
...
fn compute_entries(
    ...
    if recursive {
        let rename_options = git_diff_merge::RenameDetectionOptions {
            base: git_diff_merge::DiffNameStatusOptions {
        ...
        let mut entries = match left {
            Some(left) => git_diff_merge::diff_name_status_trees_with_rename_options(
                db,
                format,
                left,
                right,
...
fn top_level_changes(
    ...
    for name in names {
        ...
        let status = match (l, r) {
            (None, Some(_)) => git_diff_merge::NameStatus::Added,
            (Some(_), None) => git_diff_merge::NameStatus::Deleted,
            (Some(l), Some(r)) if l != r => git_diff_merge::NameStatus::Modified,
            _ => continue,
        };
        changes.push(git_diff_merge::NameStatusEntry {
            status,
            path: name,
            old_path: None,
...
///
/// This mirrors `git_diff_merge`'s recursive detection (exact-OID first, then
...
fn detect_top_level_renames(
    mut changes: Vec<git_diff_merge::NameStatusEntry>,
    db: &FileObjectDatabase,
    options: &DiffTreeOptions,
) -> Vec<git_diff_merge::NameStatusEntry> {
...
fn entry_is_blob_old(entry: &git_diff_merge::NameStatusEntry) -> bool {
    entry.old_mode.is_some_and(|mode| mode != 0o040000)
}

fn entry_is_blob_new(entry: &git_diff_merge::NameStatusEntry) -> bool {
...
fn detect_top_level_rename_pass(
    changes: Vec<git_diff_merge::NameStatusEntry>,
    db: &FileObjectDatabase,
    threshold: u8,
    rename_empty: bool,
) -> Vec<git_diff_merge::NameStatusEntry> {
    let deleted: Vec<usize> = changes
        .iter()
        .enumerate()
        .filter(|(_, e)| e.status == git_diff_merge::NameStatus::Deleted && entry_is_blob_old(e))
        .map(|(idx, _)| idx)
        .collect();
    let added: Vec<usize> = changes
        .iter()
        .enumerate()
        .filter(|(_, e)| e.status == git_diff_merge::NameStatus::Added && entry_is_blob_new(e))
        .map(|(idx, _)| idx)
        .collect();
    ...
    if threshold <= 100 {
        ...
        for (si, &src_idx) in deleted.iter().enumerate() {
            ...
            for (di, &dst_idx) in added.iter().enumerate() {
                ...
                let score = git_diff_merge::blob_similarity(&src_bytes, &dst_bytes);
                if score >= threshold {
...
fn apply_rename_assignments(
    ...
    for (idx, entry) in changes.into_iter().enumerate() {
        ...
        if let Some((src_idx, score)) = assigned.get(&idx) {
            ...
            result.push(git_diff_merge::NameStatusEntry {
...
fn detect_top_level_copy_pass(
    ...
    let sources: Vec<CopySource> = changes
        ...
        .filter(|entry| match entry.status {
            git_diff_merge::NameStatus::Deleted | git_diff_merge::NameStatus::Modified => true,
    ...
    for entry in changes.iter_mut() {
        if entry.status != git_diff_merge::NameStatus::Added || !entry_is_blob_new(entry) {
            continue;
        }
        ...
        if let Some(source) = sources.iter().find(|source| source.oid == dst_oid) {
            entry.status = git_diff_merge::NameStatus::Copied(100);
        ...
        if let Some((score, source)) = best {
            entry.status = git_diff_merge::NameStatus::Copied(score);
            entry.old_path = Some(source.path.clone());
            entry.old_mode = Some(source.mode);
            entry.old_oid = Some(source.oid.clone());
        }
...
fn sort_entries_by_path(entries: &mut [git_diff_merge::NameStatusEntry]) {
...
fn collect_changed_tree_nodes(
    ...
    prefix: Vec<u8>,
    out: &mut Vec<git_diff_merge::NameStatusEntry>,
) -> Result<()> {
    ...
    for name in names {
        ...
        let status = match (l, r) {
            (None, Some(_)) => git_diff_merge::NameStatus::Added,
            (Some(_), None) => git_diff_merge::NameStatus::Deleted,
            _ => git_diff_merge::NameStatus::Modified,
        };
        out.push(git_diff_merge::NameStatusEntry {
            status,
            path: path.clone(),
            old_path: None,
