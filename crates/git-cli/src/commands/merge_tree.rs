...
//!   a full 3-way merge (reusing [`git_diff_merge::merge_blobs`] for file content
//!   and [`git_rev::merge_bases`] for ancestry), writes the resulting top-level
...
fn run_real_merge(options: &MergeTreeOptions) -> Result<()> {
    ...
    let (ours_tree, theirs_tree, base_tree) = match &options.merge_base {
        ...
        None => {
            ...
            let bases = git_rev::merge_bases(&git_dir, format, &db, &ours_commit, &theirs_commit)?;
...
fn resolve_commit_ish(
    ...
    match git_rev::peel_to_commit(db, format, &oid) {
        Ok(commit) => Ok(commit),
        Err(_) => {
            eprintln!(
...
fn resolve_tree_ish(
    ...
    match git_rev::peel_to_tree(db, format, &oid) {
        Ok(tree) => Ok(tree),
...
/// Core 3-way tree merge. Reuses [`git_diff_merge::merge_blobs`] for textual
...
fn merge_trees(
    ...
    for path in all_paths {
            ...
            (content_mergeable, &ours, &theirs)
        {
            ...
            let result = git_diff_merge::merge_blobs(
                &base_bytes,
                &ours_bytes,
                &theirs_bytes,
...
fn write_unified_hunks(out: &mut impl Write, old: &[u8], new: &[u8]) -> Result<()> {
    let old_lines = git_diff_merge::split_lines(old);
    let new_lines = git_diff_merge::split_lines(new);
    let ops = git_diff_merge::myers_diff_lines(&old_lines, &new_lines);
    ...
    for op in &ops {
        match op {
            git_diff_merge::DiffOp::Equal(count) => {
                for _ in 0..*count {
                    rows.push(Row {
                        tag: Tag::Ctx,
            ...
            git_diff_merge::DiffOp::Delete(count) => {
                for _ in 0..*count {
                    rows.push(Row {
                        tag: Tag::Del,
                        line: old_lines[old_cursor].bytes_without_newline(),
            ...
            git_diff_merge::DiffOp::Insert(count) => {
                for _ in 0..*count {
                    rows.push(Row {
                        tag: Tag::Add,
