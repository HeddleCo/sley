...
//!
//! The merge engine reuses `git_diff_merge`: the common path (default conflict
//! style, default marker size, no conflict-resolution favoring) is produced by
//! [`git_diff_merge::merge_blobs`] directly. The extended flags — `--ours`,
...
/// Validate a `--diff-algorithm` name. git_diff_merge always merges with Myers,
...
/// favoring) the work is delegated straight to [`git_diff_merge::merge_blobs`];
...
fn merge_three_way(
    ...
    let content = if default_config {
        ...
        git_diff_merge::merge_blobs(
            base,
            ours,
            theirs,
            &git_diff_merge::MergeBlobOptions {
                ours_label,
                theirs_label,
                base_label,
                style: git_diff_merge::ConflictStyle::Merge,
            },
        )
        .content
    } else {
...
/// each side's lines.
enum Region<'a> {
    Stable(Vec<git_diff_merge::DiffLine<'a>>),
    Conflict {
        ours: Vec<git_diff_merge::DiffLine<'a>>,
        base: Vec<git_diff_merge::DiffLine<'a>>,
        theirs: Vec<git_diff_merge::DiffLine<'a>>,
    },
}

/// Walk base/ours/theirs in lockstep and classify each span, mirroring
/// `git_diff_merge::merge_blobs`' own algorithm so conflict counting and region
...
fn build_regions<'a>(base: &'a [u8], ours: &'a [u8], theirs: &'a [u8]) -> Vec<Region<'a>> {
    let base_lines = git_diff_merge::split_lines(base);
    let ours_lines = git_diff_merge::split_lines(ours);
    let theirs_lines = git_diff_merge::split_lines(theirs);
...
/// stream).
fn push_stable<'a>(regions: &mut Vec<Region<'a>>, lines: &[git_diff_merge::DiffLine<'a>]) {
    if lines.is_empty() {
        return;
    }
...
fn classify_changed_region<'a>(
    ...
    base_region: &[git_diff_merge::DiffLine<'a>],
    our_region: &[git_diff_merge::DiffLine<'a>],
    their_region: &[git_diff_merge::DiffLine<'a>],
) {
...
fn matching_regions(
    base: &[git_diff_merge::DiffLine<'_>],
    side: &[git_diff_merge::DiffLine<'_>],
) -> Vec<MatchRegion> {
    let ops = git_diff_merge::myers_diff_lines(base, side);
    let mut regions = Vec::new();
    let mut base_idx = 0usize;
    ...
    for op in ops {
        match op {
            ...
            git_diff_merge::DiffOp::Delete(n) => base_idx += n,
            git_diff_merge::DiffOp::Insert(n) => side_idx += n,
        }
    }
    regions
}
...
fn emit_lines(out: &mut Vec<u8>, lines: &[git_diff_merge::DiffLine<'_>]) {
    for line in lines {
...
fn common_prefix_len(
    a: &[git_diff_merge::DiffLine<'_>],
    b: &[git_diff_merge::DiffLine<'_>],
) -> usize {
...
fn emit_conflict(
    out: &mut Vec<u8>,
    ours: &[git_diff_merge::DiffLine<'_>],
    base: &[git_diff_merge::DiffLine<'_>],
    theirs: &[git_diff_merge::DiffLine<'_>],
    style: MergeStyle,
    marker_size: usize,
    ours_label: &str,
    base_label: &str,
