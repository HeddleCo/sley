...
fn read_shortlog_from_revisions(
    ...
    for rev in includes {
        ...
        starts.push(git_rev::peel_to_commit(&db, format, &oid)?);
    }
    for (left, right, not) in linear_ranges {
        let left_oid = resolve_revision(&git_dir, format, left)?;
        let left_oid = git_rev::peel_to_commit(&db, format, &left_oid)?;
        let right_oid = resolve_revision(&git_dir, format, right)?;
        let right_oid = git_rev::peel_to_commit(&db, format, &right_oid)?;
        if not {
    ...
    for rev in excludes {
        let oid = resolve_revision(&git_dir, format, rev)?;
        let oid = git_rev::peel_to_commit(&db, format, &oid)?;
        for record in rev_list_walk_commits(&db, format, [oid], false)? {
            excluded.insert(record.oid);
        }
    }
