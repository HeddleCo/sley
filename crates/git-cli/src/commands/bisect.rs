...
fn resolve_skip_range(repo: &BisectRepo, spec: &str) -> Result<Vec<ObjectId>> {
    ...
    if !left.is_empty() {
        ...
        for record in git_rev::walk_commits(&db, repo.format, [left_oid])? {
            excluded.insert(record.oid);
        }
    }
    let mut out = Vec::new();
    for record in git_rev::walk_commits(&db, repo.format, [right_oid])? {
...
fn bisect_step(repo: &BisectRepo, terms: &BisectTerms, no_checkout: bool) -> Result<()> {
    ...
    let mut excluded: HashSet<ObjectId> = HashSet::new();
    for good in &goods {
        if !git_rev::is_ancestor(&repo.git_dir, repo.format, &db, good, &bad)? {
        ...
        for record in git_rev::walk_commits(&db, repo.format, [good.clone()])? {
            excluded.insert(record.oid);
        }
    }

    // Walk `bad`'s history, collecting candidates (skip the excluded set).
    let mut candidates: Vec<git_rev::CommitRecord> = Vec::new();
    let mut candidate_ids: HashSet<ObjectId> = HashSet::new();
    for record in git_rev::walk_commits(&db, repo.format, [bad.clone()])? {
        if excluded.contains(&record.oid) {
            continue;
        }
...
fn compute_candidate_weights(
    candidates: &[git_rev::CommitRecord],
    ...
    let entries = match commit.parents.first() {
        Some(parent) => {
            ...
            git_diff_merge::diff_name_status_trees_with_rename_options(
                ...
                &parent_commit.tree,
                &new_tree,
                git_diff_merge::RenameDetectionOptions::default(),
            )?
        }
...
fn commit_subject_of(repo: &BisectRepo, oid: &ObjectId) -> Result<String> {
    ...
    if object.object_type != ObjectType::Commit {
        ...
        let peeled = git_rev::peel_to_commit(&db, repo.format, oid)?;
