...
//! the `git_rev`/`git_diff_merge` re-exports, the identity/date helpers, and so
...
//! here against an in-memory byte buffer using the same `git_diff_merge` engine
...
impl Default for FormatPatchOptions {
    fn default() -> Self {
        Self {
            ...
            rename_threshold: git_diff_merge::DEFAULT_RENAME_THRESHOLD,
            copy_threshold: git_diff_merge::DEFAULT_RENAME_THRESHOLD,
            revisions: Vec::new(),
        }
    }
}
...
fn render_patch(ctx: RenderContext<'_>) -> Result<Vec<u8>> {
    ...
    out.extend_from_slice(b"-- \n");
    out.extend_from_slice(git_core::UPSTREAM_GIT_COMPAT_VERSION.as_bytes());
    out.extend_from_slice(b"\n\n");
    Ok(out)
}
...
fn format_patch_signoff(config: &GitConfig) -> Result<Vec<u8>> {
    ...
    let email = env::var("GIT_COMMITTER_EMAIL")
        ...
        .unwrap_or_else(|| "git-rs@example.invalid".to_string());
...
fn first_parent_diff_entries(
    ...
    let base = git_diff_merge::DiffNameStatusOptions {
    ...
    match commit.parents.first() {
        Some(parent_oid) => {
            ...
            git_diff_merge::diff_name_status_trees_with_rename_options(
                db,
                format,
                &parent_commit.tree,
                &commit.tree,
        ...
        None => git_diff_merge::diff_name_status_empty_tree_with_rename_options(
            db,
            format,
            &commit.tree,
            rename_options,
...
fn select_commits(
    ...
    db: &FileObjectDatabase,
    options: &FormatPatchOptions,
) -> Result<Vec<git_rev::CommitRecord>> {
    ...
    let mut selected: Vec<git_rev::CommitRecord> = walked
        .into_iter()
...
fn resolve_format_patch_commit(
    ...
    git_rev::peel_to_commit(db, format, &oid).map_err(|_| unknown_revision_error(rev))
}
...
// logic against a `Vec<u8>`, using the same `git_diff_merge` data
...
fn write_patch_diff_entry(
    ...
    match entry.status {
        git_diff_merge::NameStatus::Added => writeln_buf(out, "--- /dev/null"),
        _ => writeln_buf(out, &format!("--- {}", patch_header_path("a/", old_path))),
    }
    match entry.status {
        git_diff_merge::NameStatus::Deleted => writeln_buf(out, "+++ /dev/null"),
        _ => writeln_buf(
            out,
...
fn write_patch_mode_headers(out: &mut Vec<u8>, entry: &git_diff_merge::NameStatusEntry) {
    match entry.status {
        ...
        git_diff_merge::NameStatus::Modified
        | git_diff_merge::NameStatus::Renamed(_)
        | git_diff_merge::NameStatus::Copied(_) => {
...
fn write_patch_similarity_headers(
    ...
    match entry.status {
        ...
        git_diff_merge::NameStatus::Copied(score) => {
            writeln_buf(out, &format!("similarity index {score}%"));
...
fn write_patch_hunks(out: &mut Vec<u8>, old_content: Option<&[u8]>, new_content: Option<&[u8]>) {
    let old = git_diff_merge::split_lines(old_content.unwrap_or_default());
    let new = git_diff_merge::split_lines(new_content.unwrap_or_default());
    let ops = git_diff_merge::myers_diff_lines(&old, &new);
...
fn write_one_hunk(
    out: &mut Vec<u8>,
    tagged: &[TaggedLine<'_>],
    old_lines: &[git_diff_merge::DiffLine<'_>],
    start: usize,
    end: usize,
) {
...
fn hunk_section_heading<'a>(
    old_lines: &[git_diff_merge::DiffLine<'a>],
...
/// renames/copies.
fn diffstat_path(entry: &git_diff_merge::NameStatusEntry) -> String {
    if let Some(old_path) = &entry.old_path {
        format!(
            "{} => {}",
            status_quote_path(old_path, false),
...
fn line_change_counts(old: Option<&[u8]>, new: Option<&[u8]>) -> (usize, usize) {
    let old_lines = git_diff_merge::split_lines(old.unwrap_or_default());
    let new_lines = git_diff_merge::split_lines(new.unwrap_or_default());
    let mut inserted = 0usize;
    let mut deleted = 0usize;
    for op in git_diff_merge::myers_diff_lines(&old_lines, &new_lines) {
        match op {
            git_diff_merge::DiffOp::Insert(n) => inserted += n,
            git_diff_merge::DiffOp::Delete(n) => deleted += n,
            git_diff_merge::DiffOp::Equal(_) => {}
        }
    }
    (inserted, deleted)
}
...
fn patch_blob_oid(
    ...
    let hex = oid
        .cloned()
        .or_else(|| {
            content.and_then(|content| git_core::object_id_for_bytes(format, "blob", content).ok())
        })
        .map(|oid| oid.to_hex())
...
fn patch_mode_suffix(entry: &git_diff_merge::NameStatusEntry) -> String {
...
fn parse_format_patch_args(args: &[String]) -> Result<FormatPatchOptions> {
    ...
    while let Some(arg) = iter.next() {
        ...
        match arg.as_str() {
            ...
            // git-rs emits for the common path.
            "--no-color"
            | "--color"
            | "--no-thread"
            | "--minimal"
...
fn parse_similarity(value: &str) -> Result<u8> {
    if value.is_empty() {
        return Ok(git_diff_merge::DEFAULT_RENAME_THRESHOLD);
    }
