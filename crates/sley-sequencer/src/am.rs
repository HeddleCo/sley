//! The `git am` series engine and the rebase `--apply` backend (builtin/am.c /
//! git-rebase--am.sh).
//!
//! Stage-B relocation from the CLI: everything that executes a patch series now
//! lives with the engine — mbox intake and mailinfo parsing (via [`sley_mail`]),
//! per-patch application through the shared diff engine (straight apply,
//! `-C<n>` context fuzz, `--ignore-whitespace`, binary fragments, `--reject`),
//! the 3-way constructed-fake-ancestor fallback with rerere replay, index and
//! worktree transitions, commit formation, session state under
//! `.git/rebase-apply/` (`next`/`last`/`msg-clean`-equivalent files,
//! `author-script`, `final-commit`, `apply-opt`, …), and the
//! `--abort`/`--skip`/`--continue`/`--retry`/resession transitions.
//!
//! The CLI keeps argv parsing (`setup_am_options`), usage text, exit codes, and
//! the host-injected services below ([`AmHosts`]): hook execution
//! (`applypatch-msg`, `pre-applypatch`, `post-applypatch`, `post-rewrite`),
//! partial-clone hydration, stash primitives for the apply-backend autostash,
//! and the rerere seams.
//!
//! Byte-parity notes: diagnostics that are part of git's porcelain contract are
//! emitted here verbatim (same strings as the previous CLI home), matching the
//! established library-crate pattern (see sley-worktree, [`super::pick`] and
//! [`super::rebase_drive`]).
//!
//! This module is a faithful move; the module-level lint allowances mirror the
//! source file's (`commands/am.rs`) so error paths keep their exact control
//! flow.
...
/// Repository handles shared by every am operation. The CLI builds this from
/// its open session (git dir layout, config cascade, worktree root) and hands
/// it to the engine; nothing in here reaches back into process/session state.
pub struct AmContext<'a> {
    pub git_dir: PathBuf,
    pub common_git_dir: PathBuf,
    pub worktree_root: PathBuf,
    pub format: ObjectFormat,
    /// Effective config cascade (system + global + local + `-c` overrides).
...
impl<'a> AmContext<'a> {
    pub fn new(
        ...
        format: ObjectFormat,
        config: GitConfig,
        promisor_fetch: Option<&'a dyn PromisorObjectFetch>,
    ) -> Self {
        AmContext {
            git_dir,
            common_git_dir,
    ...
    /// The apply-series state directory (`.git/rebase-apply/`).
    pub fn state_dir(&self) -> PathBuf {
        self.git_dir.join("rebase-apply")
    }
}

/// Host services for the am engine: every process-spawning or session-bound
/// operation the engine cannot own.
#[allow(clippy::type_complexity)]
pub struct AmHosts<'a> {
    /// Run a lifecycle hook with args and an optional stdin payload. A failing
    /// `applypatch-msg` / `pre-applypatch` aborts the run (git exits 1);
    /// `post-applypatch` / `post-rewrite` results are ignored by the engine.
...
pub struct AmOptions {
    ...
    pub empty_action: AmEmptyAction,
    ...
    pub patch_format: AmPatchFormat,
...
pub fn rebase_apply_in_progress(git_dir: &Path) -> bool {
    let state_dir = git_dir.join("rebase-apply");
...
pub fn parse_am_patch_format(value: &str) -> Result<AmPatchFormat> {
    match value {
...
pub fn read_state_usize(state_dir: &Path, name: &str) -> Result<usize> {
...
impl AmApplyOpts {
    ...
    /// applied content).
    pub fn whitespace_fix(&self) -> bool {
...
fn am_index_is_dirty(ctx: &AmContext, head_oid: &ObjectId) -> Result<bool> {
    let common_git_dir: &Path = &ctx.common_git_dir;
    let format: ObjectFormat = ctx.format;
...
fn try_reject_apply(
    ...
    for patch in file_patches {
        ...
        // hunk for it either.
        if patch.is_binary {
            match sley_diff_merge::apply_binary_outcome(&db, format, patch, &base)? {
                sley_diff_merge::BinaryApply::Deletion => {
                    if let Some(old) = &patch.old_path {
                        actions.push(ApplyFileAction::Remove { path: old.clone() });
                    }
                }
                sley_diff_merge::BinaryApply::Content(content) => {
...
fn try_straight_apply(
    ...
    let promisor = ctx.promisor_fetch;
    let mut actions = Vec::new();
    let db = FileObjectDatabase::from_git_dir(common_git_dir, format);
            ...
            match fs::read(worktree_root.join(rel)) {
                ...
                Err(_) => match sparse_preimages.get(old) {
                    Some((mode, oid)) if !sley_index::is_gitlink(*mode) => {
                        merge_read_blob_with_fetch(&db, oid, promisor)?
                    }
                    _ => Vec::new(),
                },
            }
...
fn stage_and_commit(
    ...
    let db = FileObjectDatabase::from_git_dir(common_git_dir, format);
    let mut index = sley_worktree::read_repository_index(git_dir, format)?.unwrap_or(Index {
        version: 2,
        entries: Vec::new(),
        extensions: Vec::new(),
        checksum: None,
...
fn create_am_commit(
    ...
    let worktree_root: &Path = &ctx.worktree_root;
    let format: ObjectFormat = ctx.format;
    let config: &GitConfig = &ctx.config;
    ...
    let _ = (hosts.run_hook)("post-applypatch", Vec::new(), None);
    Ok(new_oid)
}
...
fn apply_three_way(
    ...
    for file in file_patches {
        ...
        let base_bytes = if file.is_new {
            ...
            lookup_patch_base_blob(&db, &index_oids, &path, &old_path, &ours_map, promisor)?
        {
            bytes
        } else {
...
fn lookup_patch_base_blob(
    ...
    old_path: &[u8],
    ours_map: &MergeTreeMap,
    fetch: Option<&dyn crate::apply::PromisorObjectFetch>,
    ...
    if let Some((_, oid)) = ours_map.get(old_path).or_else(|| ours_map.get(path)) {
        return Ok(Some(merge_read_blob_with_fetch(db, oid, fetch)?));
    }
    Ok(None)
}
...
fn run_apply_post_rewrite_hook(hosts: &AmHosts<'_>, state_dir: &Path) {
...
fn finish_rebase_apply(ctx: &AmContext, hosts: &AmHosts<'_>, state_dir: &Path) -> Result<()> {
    let git_dir: &Path = &ctx.git_dir;
    let format: ObjectFormat = ctx.format;
    let config: &GitConfig = &ctx.config;
    ...
    run_apply_post_rewrite_hook(hosts, state_dir);
    let refs = FileRefStore::new(git_dir, format);
    ...
    let _ = head_display;
    apply_rebase_autostash(ctx, hosts, state_dir)?;
...
/// Start a fresh `git am` run from fully parsed [`AmOptions`] (the CLI has
/// already applied `am.messageid`/`am.threeWay` config defaults): read the
/// mbox input, detect the patch format, populate `.git/rebase-apply/`, and
/// drive the series. With `allow_empty_resume` (an `--allow-empty` invocation
