...
pub enum FormatTier {
    ...
    OidOnly,
    /// Oids + parents + left/right marker — satisfied by [`sley_rev::CommitMetadata`].
    Metadata,
...
fn parse_describe_spec(opts: &str) -> Option<DescribeSpec> {
    ...
    for part in opts.split(',') {
        if part == "tags" {
            ...
            let v = part.strip_prefix("exclude=")?;
...
pub use log::{
    ...
    emit_compiled_log_format_metadata, emit_compiled_log_format_metadata_with_message,
    emit_compiled_stash_format, emit_log_one_token, encoding_for_name, encoding_is_none,
