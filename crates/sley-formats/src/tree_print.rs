...
use sley_object::{
    EncodedObject, ObjectType, TreeEntries, TreeEntry, TreeEntryRef, tree_entry_object_type,
};

use crate::path_quote::write_quoted_path;

/// Minimal object-read seam the tree printer needs.
///
/// `sley-formats` sits below `sley-odb` in the dependency graph (both
/// `sley-odb` and `sley-diff-merge` depend on this crate), so the concrete
/// `FileObjectDatabase` type cannot be named here; `sley-odb` implements this
/// trait for it, and callers keep passing `Option<&FileObjectDatabase>`
/// unchanged.
pub trait TreeObjectSource {
    /// Header-only read (`type`, `size`) used by `--long` / `%(objectsize)`.
    fn read_tree_object_header(&self, oid: &ObjectId) -> Result<Option<(ObjectType, u64)>>;
    /// Full object read used for recursive descent and size fallbacks.
...
pub struct TreePrintOptions<'a> {
    ...
    pub show_trees: bool,
    pub tree_only: bool,
    pub oid_abbrev: Option<usize>,
    pub format_spec: Option<&'a str>,
    pub nul: bool,
}

pub trait TreeEntryView {
    fn mode(&self) -> u32;
...
pub fn print_tree<R: TreeObjectSource + ?Sized>(
    db: Option<&R>,
    format: ObjectFormat,
    body: &[u8],
...
pub fn write_object_id_hex<W: Write + ?Sized>(
    writer: &mut W,
    oid: &ObjectId,
    width: Option<usize>,
) -> Result<()> {
...
pub fn print_tree_with_prefix<R: TreeObjectSource + ?Sized>(
    db: Option<&R>,
    format: ObjectFormat,
    body: &[u8],
    prefix: &[u8],
...
pub fn print_tree_entry_to_writer<R: TreeObjectSource + ?Sized>(
...
pub fn write_tree_path(
    ...
    if options.nul {
        ...
        write_quoted_path(writer, path, false, true)?;
    }
    Ok(())
}

pub fn write_tree_oid(
    writer: &mut impl Write,
    oid: &ObjectId,
    options: TreePrintOptions<'_>,
) -> Result<()> {
...
pub fn write_tree_entry_format<R: TreeObjectSource + ?Sized>(
    writer: &mut impl Write,
    db: Option<&R>,
    entry: &impl TreeEntryView,
    path: &[u8],
...
pub fn write_tree_format_placeholder<R: TreeObjectSource + ?Sized>(
    writer: &mut impl Write,
    db: Option<&R>,
    entry: &impl TreeEntryView,
...
pub fn format_hex_nibble(ch: char) -> Result<u8> {
    // Route through the shared nibble decoder; `try_from` keeps multi-byte
    // chars from truncating into ASCII digit ranges.
    let byte = u8::try_from(ch).map_err(|_| hex_digit_error(ch))?;
    sley_core::hex_nibble_value(byte).ok_or_else(|| hex_digit_error(ch))
}

fn hex_digit_error(ch: char) -> GitError {
    GitError::Command(format!("invalid ls-tree --format hex digit {ch}"))
}

pub fn tree_entry_size_field<R: TreeObjectSource + ?Sized>(
    db: Option<&R>,
    object_type: ObjectType,
    oid: &ObjectId,
) -> Result<String> {
    ...
    if let Some((_, size)) = db.read_tree_object_header(oid)? {
        return Ok(size.to_string());
    }
    Ok(db.read_tree_object(oid)?.body.len().to_string())
}

pub fn find_tree_entry<R: TreeObjectSource + ?Sized>(
    db: &R,
    format: ObjectFormat,
    body: &[u8],
    components: &[&str],
    ...
    for entry in TreeEntries::new(format, body) {
        ...
        let object = db.read_tree_object(&entry.oid)?;
...
mod tests {
    ...
    #[test]
    fn hex_nibble_accepts_both_cases_and_rejects_other_chars() {
        assert_eq!(format_hex_nibble('0'), Ok(0));
        assert_eq!(format_hex_nibble('9'), Ok(9));
        assert_eq!(format_hex_nibble('a'), Ok(10));
        assert_eq!(format_hex_nibble('F'), Ok(15));
        assert_eq!(
            format_hex_nibble('g')
                .err()
                .map(|err| err.to_string())
                .as_deref(),
            Some("command failed: invalid ls-tree --format hex digit g")
        );
        // Multi-byte chars must not truncate into an ASCII digit range.
        assert!(format_hex_nibble('\u{0130}').is_err());
    }
}
