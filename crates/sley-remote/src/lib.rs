...
/// Truthful equal-work counts for one object transfer generated by Sley.
///
/// These are semantic events rather than pre-rendered terminal text: embedders
/// can present them however they choose, while the CLI renders Git-compatible
/// progress bytes. Sley's current local upload-pack always generates a fresh
/// pack, so the corresponding reused/pack-reused counts are zero.
