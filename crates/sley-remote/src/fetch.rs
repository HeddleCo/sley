...
fn finalize_fetch(
    ...
    let destination_config = sley_config::read_repo_config(git_dir, None).unwrap_or_default();
    ...
    if options.dry_run {
        ...
        let (rejected, rejection) = classify_fetch_ref_rejections(git_dir, format, store, updates)?;
...
mod tests {
    ...
    #[test]
    fn promisor_connectivity_walks_new_tip_and_rejects_missing_tree() {
        let git_dir = temp_repo("promisor-connectivity-missing-tree");
        let format = ObjectFormat::Sha1;
        let db = FileObjectDatabase::from_git_dir(&git_dir, format);
        ...
        let tip = db
            .write_object(EncodedObject::new(
                ...
                Commit {
                    ...
                    committer: identity,
                    encoding: None,
                    message: b"incomplete incoming tip\n".to_vec(),
                }
                .write(),
            ))
            .expect("write incomplete incoming tip");
        let updates = [FetchRefUpdate {
        ...
        let err = validate_incoming_fetch_objects(&db, format, &updates, None, true, true)
            .expect_err("promisor connectivity must walk the new tip");

        assert!(
            err.to_string()
                .contains("remote did not send all necessary objects"),
            "unexpected connectivity error: {err}"
        );
    }
    ...
    fn non_atomic_fetch_rejects_non_fast_forward_and_force_updates_it() {
        ...
        .expect("soft rejection outcome");
        assert!(
            outcome
                .rejection
                .as_deref()
                .is_some_and(|reason| reason.contains("non-fast-forward"))
        );
