...
/// [`git_odb::FileObjectDatabase::object_ids`]); it is used to compute the
...
///
/// This mirrors `git_refs::RefTarget` without taking a dependency on that
...
mod tests {
    ...
    fn connectivity_reports_corrupt_object_hash_mismatch() {
        ...
        impl ObjectReader for LyingReader {
            fn read_object(&self, oid: &ObjectId) -> git_core::Result<EncodedObject> {
                if oid == &self.claimed {
