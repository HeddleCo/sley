...
fn start_conflict_merge(program: &str, root: &Path) {
    let output = run_output_with_identity(program, root, &["merge", "topic"]);
    assert_eq!(
        output.status.code(),
        Some(1),
        "expected conflict merge to exit 1\nstdout:\n{}\nstderr:\n{}",
        String::from_utf8_lossy(&output.stdout),
        String::from_utf8_lossy(&output.stderr)
    );
    assert!(
        root.join(".git/MERGE_HEAD").is_file(),
        "expected MERGE_HEAD after conflict merge in {}",
        root.display()
    );
}
...
#[test]
fn merge_continue_after_resolving_conflict_matches_upstream_git() {
    let root = unique_temp_dir("merge-continue-resolved");
...
#[test]
fn merge_continue_with_unmerged_entries_fails() {
    let root = unique_temp_dir("merge-continue-unmerged");
    let upstream = root.join("upstream");
    let rust = root.join("rust");
    fs::create_dir_all(&upstream).expect("create upstream repo");
    fs::create_dir_all(&rust).expect("create rust repo");
...
#[test]
fn merge_continue_without_merge_fails() {
    let root = unique_temp_dir("merge-continue-no-merge");
    let upstream = root.join("upstream");
    let rust = root.join("rust");
    fs::create_dir_all(&upstream).expect("create upstream repo");
    fs::create_dir_all(&rust).expect("create rust repo");
