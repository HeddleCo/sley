...
fn start_conflict_pull_rebase(program: &str, root: &Path) -> String {
    ...
    assert!(
        ...
        "expected no MERGE_HEAD after rebase conflict pull in {}",
        root.display()
    );
    assert!(
        root.join(".git/rebase-merge").is_dir(),
        "expected rebase-merge after rebase conflict pull in {}",
        root.display()
    );
    assert!(
        root.join(".git/REBASE_HEAD").is_file(),
        "expected REBASE_HEAD after rebase conflict pull in {}",
        root.display()
    );
    pre_pull
}
...
#[test]
fn pull_rebase_conflict_then_continue_matches_upstream_git() {
    let root = unique_temp_dir("pull-rebase-conflict-continue");
