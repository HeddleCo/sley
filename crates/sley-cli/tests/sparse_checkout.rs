...
//! one by `sley` — runs the same `sparse-checkout` invocations against each,
...
/// reference and the sley copy start from byte-identical history.
...
/// Builds a paired (git, sley) repository under a fresh temp root.
