...
#[test]
fn pull_conflict_matches_upstream_git() {
    let root = unique_temp_dir("pull-conflict");
...
#[test]
fn pull_conflict_then_abort_matches_upstream_git() {
    let root = unique_temp_dir("pull-conflict-abort");
    let origin = root.join("origin");
    let upstream = root.join("upstream");
    let rust = root.join("rust");
    fs::create_dir_all(&origin).expect("create origin repo");
