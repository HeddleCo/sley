...
#[test]
fn commit_during_resolved_merge_matches_upstream_git() {
    let root = unique_temp_dir("commit-merge-resolved");
...
#[test]
fn commit_during_merge_with_unmerged_entries_fails() {
    let root = unique_temp_dir("commit-merge-unmerged");
    let upstream = root.join("upstream");
    let rust = root.join("rust");
    fs::create_dir_all(&upstream).expect("create upstream repo");
    fs::create_dir_all(&rust).expect("create rust repo");
