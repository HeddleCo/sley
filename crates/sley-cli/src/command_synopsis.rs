...
static SYNOPSES: &[(&str, &[&str])] = &[
    ...
    ("diff-pairs", &["git diff-pairs -z [<diff-options>]"]),
    (
        "diff-tree",
        &[
    ...
    ("mktag", &["git mktag"]),
    ("mktree", &["git mktree [-z] [--missing] [--batch]"]),
    (
        "multi-pack-index",
        &[
    ...
    ("receive-pack", &["git receive-pack <git-dir>"]),
    (
        "reflog",
        &[
    ...
    ("unpack-file", &["git unpack-file <blob>"]),
    (
        "unpack-objects",
    ...
    ("upload-archive", &["git upload-archive <repository>"]),
    (
        "upload-pack",
        &[
    ...
    ("version", &["git version [--build-options]"]),
    ("whatchanged", &["git whatchanged <option>..."]),
    (
        "worktree",
        &[
...
pub(crate) fn synopsis_for(command: &str) -> Option<&'static [&'static str]> {
    SYNOPSES
        .iter()
...
/// Documentation `SYNOPSIS` block (t0450 `align_after_nl` normalizes
/// continuation indentation between adoc tabs and `-h` spaces).
pub(crate) fn print_command_synopsis(command: &str) {
    let Some(lines) = synopsis_for(command) else {
    ...
    for line in lines {
        ...
        if stripped.starts_with("git ") {
            ...
            // Git's generated parse-options usage contains spaces only. Keep a
            // stable continuation indent here; t0450 normalizes the exact width
            // against Documentation but rejects literal tab bytes.
            println!("    {stripped}");
        }
    }
}
