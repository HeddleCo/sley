...
fn cmd_count_objects(args: &[String]) -> Result<()> {
    let mut verbose = false;
...
fn cmd_reflog(args: &[String]) -> Result<()> {
...
fn cmd_pack_refs(args: &[String]) -> Result<()> {
...
fn cmd_mktree(args: &[String]) -> Result<()> {
...
fn cmd_multi_pack_index(args: &[String]) -> Result<()> {
    let Some(subcommand) = args.first().map(String::as_str) else {
        return Err(GitError::Command(
...
fn cmd_rev_list(args: &[String]) -> Result<()> {
    let mut includes = Vec::new();
    let mut excludes = Vec::new();
    let mut linear_ranges = Vec::new();
    let mut symmetric_ranges = Vec::new();
...
fn cmd_update_ref(args: &[String]) -> Result<()> {
...
fn cmd_symbolic_ref(args: &[String]) -> Result<()> {
    let git_dir = discover_git_dir(env::current_dir()?)?;
    let format = repository_object_format(&git_dir)?;
    let store = FileRefStore::new(&git_dir, format);
    let mut short = false;
