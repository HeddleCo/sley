...
impl WordDiffBuffers {
    ...
    pub(crate) fn flush(&mut self, out: &mut Vec<u8>, config: &WordDiffConfig<'_>) {
        ...
        let minus_lines: Vec<sley::plumbing::sley_diff_merge::DiffLine<'_>> = minus_words
            .iter()
            .map(|span| sley::plumbing::sley_diff_merge::DiffLine {
                content: &self.minus[span.begin..span.end],
                has_newline: true,
            })
            .collect();
        let plus_lines: Vec<sley::plumbing::sley_diff_merge::DiffLine<'_>> = plus_words
            .iter()
            .map(|span| sley::plumbing::sley_diff_merge::DiffLine {
            ...
            .collect();
        let ops = sley::plumbing::sley_diff_merge::myers_diff_lines(&minus_lines, &plus_lines);
        ...
        for op in ops {
            match op {
                sley::plumbing::sley_diff_merge::DiffOp::Delete(n) => pending_del += n,
                sley::plumbing::sley_diff_merge::DiffOp::Insert(n) => pending_ins += n,
