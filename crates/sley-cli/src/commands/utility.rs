...
pub(crate) fn cmd_version(args: &[String]) -> Result<()> {
    ...
    println!("git version {}", sley::plumbing::sley_core::UPSTREAM_GIT_COMPAT_VERSION);
    if build_options {
...
pub(crate) fn cmd_bugreport(args: &[String]) -> Result<()> {
    ...
    writeln!(
        report,
        "git version {}",
        sley::plumbing::sley_core::UPSTREAM_GIT_COMPAT_VERSION
    )?;
...
fn repo_info_collect(git_dir: &Path, common_git_dir: &Path) -> Result<RepoInfo> {
    ...
    Ok(RepoInfo {
        bare: sley::plumbing::sley_worktree::worktree_root_for_git_dir(git_dir)?.is_none(),
