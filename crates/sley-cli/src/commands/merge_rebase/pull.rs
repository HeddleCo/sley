use super::*;
use {sley_core, sley_diff_merge, sley_index, sley_remote, sley_rev, sley_worktree};
...
pub(crate) fn cmd_pull(cli_session: &crate::session::CliSession, args: &[String]) -> Result<()> {
    ...
    // Fetch keeps soft ref-update rejections on the outcome so callers can
    // finish post-fetch bookkeeping. Pull must still stop before merge/rebase.
