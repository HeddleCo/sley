...
fn validate_pack_objects_options(options: &PackObjectsOptions) -> Result<()> {
    ...
    // (git auto-switches to version 1).
...
fn reject_unsupported_unpack_unreachable() -> Result<()> {
    eprintln!(
        "fatal: pack-objects --unpack-unreachable is not supported; use `git repack -A`"
    );
...
mod tests {
    ...
    #[test]
    fn name_hash_version_accepts_1_and_2_rejects_out_of_range() {
        for version in [1, 2] {
