...
pub(crate) fn cmd_commit(
    ...
    if (include_without_paths || only_without_paths || all) && (interactive || patch) {
        ...
        eprintln!("fatal: options '{other}' and '--interactive/-p/--patch' cannot be used together");
        return Err(GitError::Exit(128));
    }
    ...
    let only_or_reword_head_tree = (only_without_paths
        && pathspec_args.is_empty()
        && (amend || allow_empty))
...
fn copy_notes_for_amend(
    ...
    for reference in store.list_refs()? {
        ...
        let dest_blob =
            sley_notes::read_note_for(git_dir, format, &store, &notes_ref, new_oid)?;
        if dest_blob == Some(source_blob) {
            continue;
        }
        let source =
            sley_notes::read_note_bytes(git_dir, format, &store, &notes_ref, old_oid)?
                .unwrap_or_default();
...
fn commit_message_rest_is_empty(message: &[u8]) -> bool {
    ...
    while i < message.len() {
        ...
        i = if eol < message.len() { eol + 1 } else { message.len() };
    }
    true
}
