...
pub fn hash_object_parity_for_format(
    ...
    for (idx, case) in cases.iter().enumerate() {
        let rust = git_core::object_id_for_bytes(format, &case.object_type, &case.body)?.to_hex();
...
fn single_blob_pack_read_parity_for_format(format: ObjectFormat) -> Result<PackReadParity> {
    let root = unique_temp_dir("git-rs-pack-read");
...
fn single_blob_pack_index_parity_for_format(format: ObjectFormat) -> Result<PackIndexParity> {
    let root = unique_temp_dir("git-rs-pack-index");
    fs::create_dir_all(&root)?;
    let result = (|| -> Result<PackIndexParity> {
        ...
        let oid = git_core::ObjectId::from_hex(format, &oid)?;
...
fn delta_pack_read_parity_for_format(format: ObjectFormat) -> Result<DeltaPackReadParity> {
    let root = unique_temp_dir("git-rs-pack-delta");
    fs::create_dir_all(&root)?;
...
pub fn thin_pack_read_parity() -> Result<ThinPackReadParity> {
    ...
    let result = (|| -> Result<ThinPackReadParity> {
        ...
        let base_commit = git_sequencer::commit_index(
            root.join(".git"),
            format,
        ...
        let changed_commit = git_sequencer::commit_index(
            root.join(".git"),
            format,
            git_sequencer::CommitIndexOptions {
                author: identity.clone(),
        ...
        .oid
        .to_hex();
        let base_oid = git_core::object_id_for_bytes(format, "blob", &base_body)?.to_hex();
        let changed_oid = git_core::object_id_for_bytes(format, "blob", &changed_body)?.to_hex();
...
fn format_show_refs(refs: &[git_refs::Ref], prefix: &str) -> String {
...
fn format_show_refs_deref(
    ...
    for reference in refs {
        ...
        if object.object_type == ObjectType::Tag {
            let peeled = git_rev::peel_tags(db, format, oid)?;
            push_show_ref_line(
                &mut out,
                &peeled,
...
fn rust_pack_write_interop_parity_for_format(format: ObjectFormat) -> Result<PackWriteParity> {
    let root = unique_temp_dir("git-rs-pack-write");
    fs::create_dir_all(&root)?;
...
fn rust_bundle_write_interop_parity_for_format(format: ObjectFormat) -> Result<BundleWriteParity> {
    let root = unique_temp_dir("git-rs-bundle-write");
...
fn rust_delta_pack_write_interop_parity_for_format(
    ...
    let root = unique_temp_dir("git-rs-pack-write-delta");
    fs::create_dir_all(&root)?;
...
pub fn sha256_loose_object_interop_parity() -> Result<LooseObjectInteropParity> {
    let root = unique_temp_dir("git-rs-sha256-loose");
    fs::create_dir_all(&root)?;
...
fn packed_odb_read_interop_parity_for_format(
    ...
    let root = unique_temp_dir("git-rs-packed-odb");
    fs::create_dir_all(&root)?;
...
fn delta_packed_odb_read_interop_parity_for_format(
    ...
    let root = unique_temp_dir("git-rs-packed-odb-delta");
    fs::create_dir_all(&root)?;
    let result = (|| -> Result<DeltaPackReadParity> {
        init_repo_for_format(&root, format)?;
        let base_body = repeated_blob_body("common payload\n", "base payload\n");
...
pub fn repository_config_interop_parity() -> Result<ConfigInteropParity> {
    let root = unique_temp_dir("git-rs-config-interop");
    fs::create_dir_all(&root)?;
...
fn format_short_status_z(entries: &[git_worktree::ShortStatusEntry]) -> Vec<u8> {
    let mut out = Vec::new();
    for entry in entries {
...
fn format_short_status_branch_z(
    branch_header: &str,
    entries: &[git_worktree::ShortStatusEntry],
) -> Vec<u8> {
    let mut out = Vec::new();
...
pub fn rev_parse_object_format_parity() -> Result<RevParseObjectFormatParity> {
    let root = unique_temp_dir("git-rs-rev-parse-object-format");
    fs::create_dir_all(&root)?;
...
fn upstream_git_hash_object(
    ...
    let root = unique_temp_dir("git-rs-hash-object-sha256");
    fs::create_dir_all(&root)?;
