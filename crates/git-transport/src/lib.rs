use git_core::{Capability, GitError, ObjectFormat, ObjectId, Result};
...
mod tests {
    ...
    fn service_request_parses_and_encodes_host_and_protocol() {
        let payload =
            b"git-upload-pack /project.git\0host=example.com\0\0version=2\0agent=git-rs\0";
        ...
        assert_eq!(
            ...
            ServiceRequest {
                ...
                extra_parameters: vec!["agent=git-rs".into()],
            }
        );
    ...
    fn git_protocol_header_parses_and_encodes_extra_parameters() {
        let header = GitProtocolHeader {
            protocol: Some(ProtocolVersion::V2),
            extra_parameters: vec!["agent=git-rs".into(), "trace".into()],
        };
        assert_eq!(
            encode_git_protocol_header(&header).unwrap().as_deref(),
            Some("version=2:agent=git-rs:trace")
        );
        assert_eq!(
            parse_git_protocol_header("version=2:agent=git-rs:trace").unwrap(),
            header
        );
        assert_eq!(
        ...
        assert_eq!(
            parse_git_protocol_header("agent=git-rs").unwrap(),
            GitProtocolHeader {
                protocol: None,
                extra_parameters: vec!["agent=git-rs".into()],
            }
