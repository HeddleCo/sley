//! `git` — the ergonomic facade over the git-rs engine.
//!
...
//! underlying plumbing objects ([`git_odb::FileObjectDatabase`],
//! [`git_refs::FileRefStore`], [`git_config::GitConfig`]) on demand.
//!
...
// so the common path (`use git::{Repository, ObjectId, ...}`) stays short.
...
impl Repository {
    ...
    pub fn init_with_format(
        ...
        let layout = git_formats::RepositoryLayout::init_at(path, format, bare)?;
        Self::from_git_dir(layout.git_dir)
    }

    fn from_git_dir(git_dir: PathBuf) -> Result<Self> {
        let common_dir = git_odb::repository_common_dir(&git_dir);
    ...
    pub fn rev_parse(&self, spec: &str) -> Result<ObjectId> {
        git_rev::resolve_revision(&self.git_dir, self.format, spec)
    }
...
mod tests {
    ...
    fn plumbing_reexports_are_reachable() {
        ...
        let _format: plumbing::git_core::ObjectFormat = ObjectFormat::Sha1;
        let _: fn(&[u8]) -> Result<plumbing::git_config::GitConfig> = plumbing::git_config::GitConfig::parse;
    }
}
