...
cargo run -p git-cli -- stash show [-u|--include-untracked|--only-untracked] [--raw|--stat|--compact-summary|--numstat|--shortstat|--summary|--name-only|--name-status|-p|--patch|--patch-with-raw|--patch-with-stat|--oneline|--quiet] [stash@{<n>}]
