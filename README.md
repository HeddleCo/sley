...
cargo run -p git-cli -- stash list [--oneline|--format=<format>|--pretty=format:<format>] [--grep=<pattern>] [--all-match|--invert-grep|-i|--regexp-ignore-case|-F|--fixed-strings] [--abbrev[=<n>]|--no-abbrev] [-<n>|-n <n>|--max-count=<n>]
cargo run -p git-cli -- stash clear
cargo run -p git-cli -- stash drop [-q|--quiet|--no-quiet] [stash@{<n>}]
cargo run -p git-cli -- stash show [-u|--include-untracked|--only-untracked] [--diff-filter=<filter>] [--raw|--stat|--compact-summary|--numstat|--shortstat|--summary|--name-only|--name-status|-p|--patch|--patch-with-raw|--patch-with-stat|--oneline|--quiet] [stash@{<n>}]
